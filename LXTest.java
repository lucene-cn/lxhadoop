...
//					//结果比对
					...
					pos=(long) (Math.random()*max_position);
					for(int j=0;j<batchCnt;j++)
					{
						position[j]=1+pos+(long) (Math.random()*1024*1024*128);//随机数不能越界 //1+(long) (Math.random()*max_position);//
					...
					System.out.println("cmpdiff:比对结果:"+cmp_result+"@校验成功次数:"+cnt+"@读取次数:"+batchCnt+"@优化后时间:"+(ts1-ts)+"@原始hdfs时间:"+(ts4-ts3));

					
				}else {
					System.out.println("cmpdiff:读取次数:"+batchCnt+"@优化后时间:"+(ts1-ts));
