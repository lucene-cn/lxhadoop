...
public class PacketReceiverBatch implements Closeable {
  ...
  static final Logger LOG = LoggerFactory.getLogger(PacketReceiverBatch.class);
   ...
   */
  private PacketHeaderBatch curHeader;

  public PacketReceiverBatch(boolean useDirectBuffers) {
    this.useDirectBuffers = useDirectBuffers;
    reallocPacketBuf(PacketHeaderBatch.PKT_LENGTHS_LEN);
  }

  public PacketHeaderBatch getHeader() {
    return curHeader;
  }
  ...
  private void doRead(ReadableByteChannel ch, InputStream in)
    ...
    curPacketBuf.clear();
    curPacketBuf.limit(PacketHeaderBatch.PKT_LENGTHS_LEN);
    ...
    reallocPacketBuf(PacketHeaderBatch.PKT_LENGTHS_LEN +
        dataPlusChecksumLen + headerLen);
    curPacketBuf.clear();
    curPacketBuf.position(PacketHeaderBatch.PKT_LENGTHS_LEN);
    curPacketBuf.limit(PacketHeaderBatch.PKT_LENGTHS_LEN +
        dataPlusChecksumLen + headerLen);
    doReadFully(ch, in, curPacketBuf);
    curPacketBuf.flip();
    ...
    if (curHeader == null) {
      curHeader = new PacketHeaderBatch();
    }
  ...
  private void reslicePacket(
    ...
    //   |----------- lenThroughChecksums   ----|
    //   |------------------- lenThroughData    ------|
    int lenThroughHeader = PacketHeaderBatch.PKT_LENGTHS_LEN + headerLen;
